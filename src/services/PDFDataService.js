...
class PDFDataService {
    ...
    _normalizeMetadata(proposal, client) {
        ...
        return {
            ...
            validade: proposal.validade
                ? this.dateFormatter.format(new Date(proposal.validade))
                : '',
    ...
    _calculateTotals(proposal) {
        ...
        const subtotal_itens = items.reduce(
            (sum, item) => sum + this._calculateItemSubtotal(item),
            0
        )
        const subtotal_insumos = supplies.reduce(
            (sum, supply) => sum + this._calculateItemSubtotal(supply),
            0
        )
