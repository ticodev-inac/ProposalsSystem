<template>
    <div class="opcionais-page">
            ...
            <div v-else>
                <div v-for="item in filteredItems" :key="item.key" class="grid-row">
                    ...
                    <div class="grid-cell">
                        <span
                            class="status-badge"
                            :class="item.categoria === 'Produto' ? 'ativo' : 'inativo'"
                        >
                            {{ item.categoria }}
                        </span>
                    </div>
        ...
        <div class="opcionais-grid" v-if="!currentProposalId">
            ...
            <div v-else>
                <div v-for="opcional in filteredOpcionais" :key="opcional.id" class="grid-row">
        ...
        setup() {
            ...
            const filterOpcionais = () => {
                ...
                if (selectedStatus.value) {
                    filtered = filtered.filter(
                        (opcional) => opcional.status === selectedStatus.value
                    )
                }
            ...
            const loadProposal = async () => {
                ...
                try {
                    ...
                    selectedOptionals.value = existing.map((it) => ({
                        ...
                        categoria:
                            it.categoria ??
                            (it.source_table === 'services' ? 'Serviço' : 'Produto'),
            ...
            const optionalsSubtotal = computed(() => {
                return selectedOptionals.value.reduce(
            ...
            const cancelEditing = () => {
                if (
                    confirm(
                        'Tem certeza que deseja cancelar a edição da proposta? As alterações não salvas serão perdidas.'
                    )
                ) {
