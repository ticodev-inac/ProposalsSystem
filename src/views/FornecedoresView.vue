    ...
    <div class="fornecedores-view">
        ...
        <div v-else class="content">
            <div class="actions">
                ...
                <button class="btn-secondary" @click="listarFornecedores" :disabled="loading">
            ...
            <div class="fornecedores-list" v-if="fornecedoresFiltrados.length > 0">
                ...
                >
                    <div class="fornecedor-info">
                        ...
                        <span class="phone" v-if="fornecedor.phone">
                            Tel: {{ fornecedor.phone }}
                        </span>
                    </div>
                    <div class="fornecedor-actions">
                        <button class="btn-edit" @click="editarFornecedor(fornecedor)">
                            Editar
                        </button>
                        <button class="btn-delete" @click="confirmarExclusao(fornecedor)">
                            Excluir
                        </button>
                    </div>
                </div>
            ...
            <div class="modal-content form-modal" @click.stop>
                ...
                <form @submit.prevent="salvarFornecedor" class="fornecedor-form">
                    ...
                    <div class="modal-actions">
                        <button type="button" class="btn-secondary" @click="fecharModalCriar">
                            Cancelar
                        </button>
        ...
        <div v-if="showDeleteModal" class="modal-overlay" @click="cancelarExclusao">
            <div class="modal-content" @click.stop>
                ...
                <p>
                    ...
                    <strong>
                        {{
                            fornecedorParaExcluir?.company_name ||
                            fornecedorParaExcluir?.contact_name
                        }}
                    </strong>
                    ?
                </p>
                <div class="modal-actions">
                    ...
                    >
                        {{ deletingSupplier ? 'Excluindo...' : 'Excluir' }}
                    </button>
                </div>
            </div>
        </div>
        ...
        setup() {
            ...
            const fornecedoresFiltrados = computed(() => {
                ...
                return fornecedores.value.filter(
                    (fornecedor) =>
                        (fornecedor.company_name &&
                            fornecedor.company_name.toLowerCase().includes(term)) ||
                        (fornecedor.contact_name &&
                            fornecedor.contact_name.toLowerCase().includes(term)) ||
            ...
            const salvarFornecedor = async () => {
                try {
                    ...
                    if (novoFornecedorData.value.cnpj) {
                        const cnpjExiste = fornecedores.value.some(
                            (f) => f.cnpj === novoFornecedorData.value.cnpj
                        )
                        if (cnpjExiste) {
                    ...
                    if (novoFornecedorData.value.email) {
                        const emailExiste = fornecedores.value.some(
                            (f) => f.email === novoFornecedorData.value.email
                        )
                        if (emailExiste) {
                    ...
                    const novoFornecedor = await databaseStore.createSupplier(
                        novoFornecedorData.value
                    )
            ...
            const listarFornecedores = async () => {
                try {
                    ...
                    error.value =
                        'Erro ao carregar fornecedores. Verifique a conexão com o banco de dados.'
                    fornecedores.value = []
                } finally {
                    loading.value = false
                }
            ...
            const atualizarFornecedor = async () => {
                try {
                    ...
                    if (fornecedorEditando.value.cnpj) {
                        const cnpjExiste = fornecedores.value.some(
                            (f) =>
                                f.cnpj === fornecedorEditando.value.cnpj &&
                                f.id !== fornecedorEditando.value.id
                        )
                        if (cnpjExiste) {
                    ...
                    if (fornecedorEditando.value.email) {
                        const emailExiste = fornecedores.value.some(
                            (f) =>
                                f.email === fornecedorEditando.value.email &&
                                f.id !== fornecedorEditando.value.id
                        )
                        if (emailExiste) {
                    ...
                    const fornecedorAtualizado = await databaseStore.updateSupplier(
                        fornecedorEditando.value.id,
                        fornecedorEditando.value
                    )

                    // Atualizar na lista local
                    const index = fornecedores.value.findIndex(
            ...
            const excluirFornecedor = async () => {
                try {
                    ...
                    fornecedores.value = fornecedores.value.filter(
