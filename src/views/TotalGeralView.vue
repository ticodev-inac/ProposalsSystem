    ...
    <div class="optionals-control">
      <div class="control-section">
        ...
        <p class="control-description">
          Os itens opcionais são complementos que podem ser incluídos ou não na proposta final
        </p>
      </div>
    </div>
        ...
        <div class="breakdown-section">
          ...
          <div class="breakdown-table">
            ...
            <div class="breakdown-row" v-show="incluirOpcionais">
              ...
              <span class="breakdown-value">{{ formatCurrency(totals.opcionais) }}</span>
            </div>
            ...
            <div class="breakdown-row">
              ...
              <span class="breakdown-value discount">-{{ formatCurrency(discountAmount) }}</span>
            </div>
...
export default {
  ...
  setup() {
    ...
    const canAdvance = computed(() => {
      return totals.value.produtos > 0 || totals.value.servicos > 0 || totals.value.insumos > 0
    })
    ...
    const calculateTotals = async () => {
      ...
      try {
        if (currentProposalId.value && currentProposal.value) {
          ...
          totals.value.produtos = produtos?.reduce((sum, item) => sum + (item.price || 0), 0) || 0
          ...
          totals.value.servicos = servicos?.reduce((sum, item) => sum + (item.price || 0), 0) || 0
          ...
          totals.value.insumos = insumos?.reduce((sum, item) => sum + (item.price || 0), 0) || 0
          ...
          totals.value.opcionais = opcionais?.reduce((sum, item) => sum + (item.price ?? item.valor ?? 0), 0) || 0
    ...
    const saveSettings = async () => {
      try {
        ...
        if (currentProposalId.value) {
          await supabase
            .from('proposals')
            .update({ total_amount: Number(totalGeral.value) || 0, updated_at: new Date().toISOString() })
