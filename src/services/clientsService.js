...
class ClientsService {
  ...
  async createClient(clientData) {
    try {
      ...
      if (!companyId) {
        throw new Error('Não foi possível determinar company_id. Informe company_name ao criar o cliente (para criarmos a company automaticamente) ou defina um company_id.')
      }
  ...
  async searchClients(searchTerm) {
    try {
      const { data, error } = await this.supabase
        .from(this.tableName)
        .select('*')
        .or(`company_name.ilike.%${searchTerm}%,cnpj.ilike.%${searchTerm}%,email.ilike.%${searchTerm}%`)
        .order('company_name', { ascending: true })
